...
def downloadSteam(videoData):
    ...
    # 记录初始文件大小
    if videoData['title'] not in VIDEO_TOTOL_SIZE:
        VIDEO_TOTOL_SIZE[videoData['title']] = content_size

    # print('开始下载：', videoData['title'], '总大小{size:.2f}M'.format(size = content_size / chunk_size / chunk_size))
    index = 0
